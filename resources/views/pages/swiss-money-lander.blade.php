@php
    ...
    $cluster = $cluster ?? (
        class_exists(\App\Support\SwissMoneyLanders::class)
        && method_exists(\App\Support\SwissMoneyLanders::class, 'clusterLinks')
            ? \App\Support\SwissMoneyLanders::clusterLinks($slug)
            : []
    );
    $hreflangLocales = 'ch';
    $hreflangXDefault = 'de';
...
<div class="container py-5" style="max-width: 1040px;">
            ...
            @if(!empty($priceFrom))
                <div class="col-md-6">
                    <div class="h-100 p-4 rounded-4 bg-white border">
                        ...
                        <p class="small text-muted mb-0 mt-2">Niedrigster Checkout-Preis auf geprüften, aktiven Schweiz-Listings in diesem Moment. Keine feste Preisliste.</p>
                    </div>
                </div>
            @endif
            @if(!empty($siteCount))
                <div class="col-md-6">
                    <div class="h-100 p-4 rounded-4 bg-white border">
                        ...
                        <p class="small text-muted mb-0 mt-2">Aktive, geprüfte Publisher mit Primärland Schweiz im Katalog, wenn die Zahl verfügbar ist.</p>
                    </div>
                </div>
            @endif
        </div>
    ...
    @if(isset($teasers) && $teasers->isNotEmpty())
        ...
        <p class="small text-muted mb-5">Wir zeigen DA, DR und den Preis in Euro, wenn sie am Listing existieren. Erfundene Kennzahlen erscheinen nicht. Die Tabelle listet Primärland Schweiz — keine DACH-Mischung.</p>
    @endif
    ...
    @if($faqs !== [])
        <section class="mb-5" aria-labelledby="ch-money-faq">
            <h2 id="ch-money-faq" class="h4 mb-3" style="color:#1a585e;">{{ __('messages.nav_faq') }}</h2>
            @foreach($faqs as $faq)
                <div class="mb-3">
