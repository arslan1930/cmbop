...
class AdminOrdersConsoleTest extends TestCase
{
    ...
    public function test_order_show_uses_named_dispute_routes_and_layout_sweetalert(): void
    {
        ...
        $this->actingAs($admin)
            ->get(route('admin.orders.show', $order->id))
            ->assertOk()
            ->assertSee('data-resolve-url="'.route('admin.orders.disputes.uphold', $dispute, false).'"', false)
            ->assertDontSee('data-resolve-url="'.route('admin.orders.disputes.uphold', $dispute).'"', false)
